        ...
        try:            
            ...
            if not target_cols:
                ...
                if len(grouped_renewable) == 0:
                    ...
                    # Always calculate total renewable volume (for growth line)
                    grouped_renewable['Total_Renewable'] = grouped_renewable[target_cols].sum(axis=1, skipna=True)
                    
                    # Calculate which columns to display based on selected energy type
                    ...
                    # Growth calculations - ALWAYS based on Total_Renewable
                    if renewable_growth_type == "Year-over-Year (YoY)":
                        if renewable_period == "Monthly":
                            grouped_renewable['Growth'] = grouped_renewable['Total_Renewable'].pct_change(periods=12) * 100
                        elif renewable_period == "Quarterly":
                            grouped_renewable['Growth'] = grouped_renewable['Total_Renewable'].pct_change(periods=4) * 100
                        elif renewable_period == "Semi-Annual":
                            grouped_renewable['Growth'] = grouped_renewable['Total_Renewable'].pct_change(periods=2) * 100
                        else:  # Annual
                            grouped_renewable['Growth'] = grouped_renewable['Total_Renewable'].pct_change(periods=1) * 100
                    else:  # YTD
                        grouped_renewable['Growth'] = calculate_ytd_growth(grouped_renewable, 'Total_Renewable', 'Date', renewable_period)
                    
                    # Create proper time axis labels
                    if 'Date' in grouped_renewable.columns:
                        if renewable_period == "Monthly":
                            time_labels = [d.strftime('%b %Y') for d in grouped_renewable['Date']]
                        elif renewable_period == "Quarterly":
                            time_labels = [f"Q{d.quarter} {d.year}" for d in grouped_renewable['Date']]
                        elif renewable_period == "Semi-Annual":
                            time_labels = [f"H{((d.month-1)//6)+1} {d.year}" for d in grouped_renewable['Date']]
                        else:  # Annual
